...
def generate_pdf_html(data):
    ...
    th, td { padding: 5px; font-size: 14px; text-align: center; display: table-cell; }
    ...
    for page_index, page_data in enumerate(pages):
        ...
        # customBorders를 셀별로 매핑
        border_map = {}
        for border in custom_borders:
            row, col = border["row"], border["col"]
            border_map[(row, col)] = {
                "top": border.get("top", {}),
                "bottom": border.get("bottom", {}),
                "left": border.get("left", {}),
                "right": border.get("right", {})
            }
        ...
        for row_index, row in enumerate(table_data):
            ...
            for col_index, cell in enumerate(row):
                ...
                align_value = alignment.get("align", "htCenter htMiddle")
                ...
                if borders.get("top"):
                    width = borders["top"].get("width", 1)
                    color = borders["top"].get("color", "black")
                    border_style += f"border-top: {width}px solid {color};"
                if borders.get("bottom"):
                    width = borders["bottom"].get("width", 1)
                    color = borders["bottom"].get("color", "black")
                    border_style += f"border-bottom: {width}px solid {color};"
                if borders.get("left"):
                    width = borders["left"].get("width", 1)
                    color = borders["left"].get("color", "black")
                    border_style += f"border-left: {width}px solid {color};"
                ...
                <td rowspan="{rowspan}" colspan="{colspan}" 
                    ...
                    text-align: {text_align}; vertical-align: {vertical_align}; {border_style}">
                    {cell_content}
                </td>'''
            html += "</tr>"
